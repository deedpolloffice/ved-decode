...
m=w.document.referrer.match(/[\/.]google\..*[?&]ved=([a-zA-Z0-9_-]+)\b/);if(_.ved=m&&m.length&&ved_decode(m[1])||null){_.linkIndex=_.ved[1]||null;_.linkType=linkTypes[_.ved[2]]||_.ved[2]||null;_.resultPosition=_.ved[6]!==undefined?_.ved[6]+1:null;_.subResultPosition=_.ved[5]!==undefined?_.ved[5]+1:null;_.page=(_.ved[7]||0)/10+1}
function ved_decode(c){var d={t:2,r:6,s:7,i:1},r={},g,m;if(c.match(/^1/)){g=/([a-z]+):([0-9]+)/ig;while((m=g.exec(c))!==null)r[d[m[1]]||m[1]]=parseInt(m[2],10);return r}c=c.replace(/_/,'+').replace('-','/');c=w.atob((c+"===").slice(1,c.length+3-(c.length+2)%4));g=/([\x80-\xff]*[\x00-\x7f])([\x80-\xff]*[\x00-\x7f])/g;while((m=g.exec(c))!==null)r[f(m[1])>>3]=f(m[2]);return r;function f(a){var b=0,i=0;for(;i<a.length;++i)b+=(a.charCodeAt(i)&0x7f)<<(i*7);return b}}
})(VedDecode,window);
